...
import type { HistoryItem, HistoryValueResolver } from "./types"
...
const getActorLabel = (item: HistoryItem) => {
...
const getSummary = (item: HistoryItem, resolver?: HistoryValueResolver) => {
  const actionType = (item.action_type || "").toUpperCase()
  const oldValues = item.old_values ?? {}
  const newValues = item.new_values ?? {}

  if (actionType === "STATUS_CHANGE") {
    const fromStatus = (oldValues as Record<string, unknown>)?.status_code ?? (oldValues as Record<string, unknown>)?.status_id ?? (oldValues as Record<string, unknown>)?.statut_id
  ...
  // Metier add/remove (artisan audit)
  if (actionType.startsWith("METIER_")) {
    const values = actionType === "METIER_REMOVE" ? oldValues : newValues
    const metierId = (values as Record<string, unknown>)?.metier_id
    const metierDisplay = metierId ? resolveDisplay("metier_id", metierId, resolver) : null
    const isPrimary = (values as Record<string, unknown>)?.is_primary as boolean | undefined
    return (
      <div className="history-item-summary-text">
        {metierDisplay && <span className="font-medium">{metierDisplay.label}</span>}
        {isPrimary && <Badge variant="outline" className="ml-2 text-[9px]">Principal</Badge>}
      </div>
    )
  }

  // Zone add/remove (artisan audit)
  if (actionType.startsWith("ZONE_")) {
    const values = actionType === "ZONE_REMOVE" ? oldValues : newValues
    const zoneId = (values as Record<string, unknown>)?.zone_id
    const zoneDisplay = zoneId ? resolveDisplay("zone_id", zoneId, resolver) : null
    return (
      <div className="history-item-summary-text">
        {zoneDisplay && <span className="font-medium">{zoneDisplay.label}</span>}
      </div>
    )
  }

  // Absence add/update/delete (artisan audit)
  if (actionType.startsWith("ABSENCE_")) {
    const values = actionType === "ABSENCE_DELETE" ? oldValues : newValues
    const startDate = (values as Record<string, unknown>)?.start_date as string | undefined
    const endDate = (values as Record<string, unknown>)?.end_date as string | undefined
    const reason = (values as Record<string, unknown>)?.reason as string | undefined
    const parsedStart = startDate ? safeParseDate(startDate) : null
    const parsedEnd = endDate ? safeParseDate(endDate) : null
    return (
      <div className="history-item-summary-text">
        {parsedStart && parsedEnd && (
          <span className="font-medium">
            {format(parsedStart, "dd/MM/yyyy", { locale: fr })} - {format(parsedEnd, "dd/MM/yyyy", { locale: fr })}
          </span>
        )}
  ...
  if (actionType === "CREATE") {
    ...
    // Artisan create
    const plainNom = (newValues as Record<string, unknown>)?.plain_nom as string | undefined
    const raisonSociale = (newValues as Record<string, unknown>)?.raison_sociale as string | undefined
    ...
    if (plainNom || raisonSociale) {
      return (
        <div className="history-item-summary-text">
          <span className="font-medium">{plainNom || raisonSociale}</span>
        </div>
      )
    }
    return null
...
export function HistoryEntry({ item, isFirst, isLast, valueResolver }: HistoryEntryProps) {
