...
export function renderUnderstatementCell({ intervention }: Pick<CellRendererArgs, "intervention">): CellRender {
  const value = intervention.understatement
  if (!value) return { content: "—" }

  const textColor = intervention.sousStatutTextColor ?? '#000000'
  const bgColor = intervention.sousStatutBgColor ?? 'transparent'
