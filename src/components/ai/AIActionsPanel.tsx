...
export function AIActionsPanel({ isOpen, onClose, onSelectAction, context }: AIActionsPanelProps) {
  ...
  if (actions.length === 0 && isOpen) {
    return (
      <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
        <DialogContent className="sm:max-w-[400px] bg-background border border-border shadow-lg rounded-lg p-0">
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[440px] bg-background border border-border shadow-lg rounded-lg p-0">
