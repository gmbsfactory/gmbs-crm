...
export function OnlineUsersBar({ users, offlineUsers = [], onSelectUser }: OnlineUsersBarProps) {
  ...
  const { activeUsers, idleUsers } = useMemo(() => {
  ...
  if (activeUsers.length === 0 && idleUsers.length === 0 && offlineUsers.length === 0) {
    return (
  ...
  return (
    <TooltipProvider delayDuration={150}>
      <div className="flex flex-wrap gap-3">
        {/* Active online users — green pulse */}
            ...
            <Tooltip key={user.userId}>
              ...
              <TooltipContent side="bottom" className="max-w-xs">
                <div className="space-y-1">
                  ...
                  <p className="text-muted-foreground">En ligne — {pageLabel}</p>
            ...
            <Tooltip key={user.userId}>
              <TooltipTrigger asChild>
                <button
                  type="button"
                  onClick={() => onSelectUser(user.userId)}
                  className="group relative flex flex-col items-center gap-1 rounded-lg px-2 py-1.5 transition-colors hover:bg-accent/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring opacity-80"
                >
                  <div className="relative">
                    <GestionnaireBadge
                      prenom={firstName}
                      ...
                      showBorder
                    />
                    {/* Orange pulse dot — idle */}
                    <span className="absolute -bottom-0.5 -right-0.5 flex h-3 w-3">
                      <span className="absolute inline-flex h-full w-full animate-pulse rounded-full bg-amber-400 opacity-75" />
                      <span className="relative inline-flex h-3 w-3 rounded-full bg-amber-500 ring-2 ring-background" />
                    </span>
                  </div>
                  <span className="text-[10px] font-medium text-muted-foreground max-w-[60px] truncate leading-tight">
                    {firstName}
              ...
              <TooltipContent side="bottom" className="max-w-xs">
                <div className="space-y-1">
                  <p className="font-semibold">{user.name}</p>
                  <p className="text-amber-600 dark:text-amber-400">Inactif — {pageLabel}</p>
                </div>
              </TooltipContent>
            </Tooltip>
          )
        ...
        {/* Separator between online (active+idle) and offline */}
        {(activeUsers.length > 0 || idleUsers.length > 0) && offlineUsers.length > 0 && (
