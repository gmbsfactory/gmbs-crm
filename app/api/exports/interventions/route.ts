...
export async function GET(req: NextRequest) {
  ...
  const { data: artisansRows } = await supabase
    ...
    .select('intervention_id, is_primary, role, artisans(plain_nom)')
    .in('intervention_id', ids)
    .order('is_primary', { ascending: false })

  const artisanMap = new Map<string, { primary: string; secondary: string }>()
  for (const row of (artisansRows ?? []) as any[]) {
    const entry = artisanMap.get(row.intervention_id) ?? { primary: '', secondary: '' }
  ...
  // Commentaires internes : nécessaires uniquement en mode étendu.
  ...
  const rows: InterventionRow[] = list.map((i: any) => {
    const artisans = artisanMap.get(i.id) ?? { primary: '', secondary: '' }
    return {
