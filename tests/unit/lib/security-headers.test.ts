...
const EXPECTED_HEADERS: Record<string, string> = {
  ...
  'Content-Security-Policy':
    "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' blob: https://vercel.live; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https:; font-src 'self' data:; connect-src 'self' https://*.supabase.co wss://*.supabase.co https://tiles.openfreemap.org https://vercel.live wss://vercel.live; worker-src 'self' blob:; frame-src 'self' https://*.supabase.co blob: https://vercel.live; frame-ancestors 'none';",
