...
| `99083_email_attachments_intervention.sql` | **Portail artisans — pièces jointes de l'e-mail** *(lot L7)* : `intervention_attachments` gagne `sent_to_artisan_at` (date du **premier** envoi à un artisan, jamais réécrite) et `sent_to_artisan_email_log_id` (FK `email_logs` `ON DELETE SET NULL` : supprimer un journal d'e-mail ne supprime jamais la pièce), plus un index **partiel** `(intervention_id, sent_to_artisan_at DESC) WHERE sent_to_artisan_at IS NOT NULL` — l'immense majorité des pièces ne partent jamais par mail. `email_logs` gagne `attachment_ids uuid[]` : la liste **exacte** des pièces de chaque envoi, là où `attachments_count` ne disait que « 3 pièces ». Durcissement `email_logs` (motif `99082`) : `REVOKE ALL FROM anon` puis `GRANT` des quatre verbes à `authenticated` — la RLS n'avait aucune policy `anon`, mais `TRUNCATE` et `TRIGGER` ne passent par aucune policy. **Idempotente**, rejouée deux fois de suite sans erreur |
...
>
> **99083 — pièces jointes de l'e-mail** *(lot L7)* : spécification [portail-vision-spec.md](../architecture/portail-vision-spec.md) §5.6. Le gestionnaire joignait des fichiers de **son disque**, encodés en base64 dans le corps de la requête ; `email_logs` n'en gardait que le nombre, et l'application artisan ne *pouvait* donc pas montrer « ce que joint l'e-mail ». Les pièces sont désormais des lignes d'`intervention_attachments` lues dans le stockage par la route d'envoi. Cette migration ne pose que la **marque** d'envoi. Deux choix à connaître : `sent_to_artisan_at` est posée **une seule fois** (le premier envoi fait foi) et l'écriture porte le filtre `… AND sent_to_artisan_at IS NULL` — sans lui, chaque relance rejouerait un `UPDATE` sur une table publiée en `REPLICA IDENTITY FULL` (`99077`), donc du WAL et un événement temps réel pour rien ; et `attachment_ids` est un tableau **sans** clé étrangère, parce qu'un tableau ne peut pas en porter et que supprimer une pièce ne doit pas réécrire un journal d'envoi. Non-régression couverte par `tests/integration/migrations/99083-email-attachments.test.ts` (base **locale** uniquement).
>
