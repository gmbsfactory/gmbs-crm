...
2. **Flux d'activité global (timeline V3)** — flux en **rail timeline** (heure + **séparateurs de section collants** : par **heure** en vue jour, par **jour** en vue semaine/mois) avec **temps relatif** « il y a … », via `useGlobalActivityFeed`. **Menu ⚙ Filtres** : **Affichage** (Groupé/Détaillé) · **Type d'événement** (Tout/Interventions/Artisans/Autre + comptes) · **Fenêtre de regroupement** (5/15/30/60 min · journée). En **Groupé**, les actions d'un **même dossier** dans la fenêtre sont fusionnées en **une carte par dossier** : si **plusieurs gestionnaires** y interviennent, leurs avatars s'**empilent** (3 max + chip **+N** dont le **hover** liste tous les intervenants et leur nombre d'actions). La carte agrège les **chips de catégories** + un **compteur**, et se **déplie** en lignes de détail (1 par action, avec l'**avatar de son auteur**, statut avant→après, diff champ-à-champ). En **Détaillé**, 1 ligne/action à plat. **Recherche** + **chips de catégorie** conservées. Les events **Connexion/Déconnexion** restent **dérivés** de `get_team_connections` (`first_seen_at` / `last_seen_at`) + présence — pas dans l'audit log. **Aucune nouvelle RPC/migration** : tout est dérivé côté client.
