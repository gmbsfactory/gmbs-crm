...
Filtre gestionnaire **multi-sélection** (chips retirables + `ExpandableAvatarGroup`, repris du Dashboard). Données : `get_team_weekly_stats` (roster + barres + stats), `get_team_connections` (timeline + sessions + connexions, par carte ouverte), `get_activity_heatmap` (heatmap), `get_global_activity_feed` (flux), `get_top_entities` (dossiers) — toutes **dérivées côté client** des RPC existantes. La seule migration ajoutée (**`99059`**) expose les journaux d'audit au **Realtime** pour le rafraîchissement live (voir Capacités → Temps réel).
...
2. **Flux d'activité global (timeline V3 · modèle « brique »)** — l'unité du flux est la **brique atomique** : 1 champ modifié, 1 coût, 1 paiement, 1 statut, 1 doc, 1 commentaire, 1 mail, 1 connexion. Chaque brique se rend **toujours pareil** : `[heure] [acteur] [glyphe] libellé · avant → après`. Concrètement un **UPDATE est éclaté en 1 brique par champ** (fini le bloc « Modification N champs » imbriqué) ; la **création reste 1 ligne** (geste unique). Les coûts/paiements sont **typés** (`cost_type` → SST/Matériel/… (+ Art. 1/2), `payment_type`) avec montant avant→après ; le statut garde son avant→après. Flux en **rail timeline** + **séparateurs de section collants** (par **heure** en vue jour, par **jour** en semaine/mois) + **temps relatif** « il y a … », via `useGlobalActivityFeed`. **Menu ⚙ Filtres** : **Affichage** (Groupé/Détaillé) · **Type d'événement** (Tout/Interventions/Artisans/Autre + comptes) · **Fenêtre de regroupement** (5/15/30/60 min · journée). En **Détaillé**, les briques sont **à plat**. En **Groupé**, elles sont regroupées en **une carte par dossier** (fenêtre temporelle) : plusieurs gestionnaires → **pile d'avatars** (3 max + chip **+N** au survol) ; la carte agrège les **chips de catégories** + un **compteur de modifications réelles** (briques), et se **déplie** sur les briques. Valeurs longues **tronquées à 50 car. + tooltip** ; champs techniques (`search_vector`…) exclus. Les **Connexion/Déconnexion** restent dérivées de `get_team_connections` + présence. **Rafraîchissement temps réel** : `useDevMonitoringRealtime` (monté par `page.tsx`, dev only) s'abonne aux **INSERT** de `intervention_audit_log` / `artisan_audit_log` (canal dédié `monitoring-dev-audit`, exposés au Realtime par la **migration `99059`**) et **invalide en debounce** les requêtes dérivées (`global-activity-feed`, `team-weekly-stats`, `team-connections`, `top-entities`, `activity-heatmap`) → le flux et les stats apparaissent **en direct**, sans rechargement. Le panneau droit est **responsive jusqu'à 25 %** de largeur (split clampé à [25,75] % côté gauche) **sans débordement horizontal** (`min-w-0` + `flex-wrap` + troncatures).

   **Enrichissement des briques** : **email** → type (`email_type` : devis / intervention) + destinataire (`recipient_email`) + **bouton artisan** ; **affectation** → « Artisan assigné/retiré » + rôle **SST 1/2** (`role`/`is_primary`) + **nom artisan** (résolu) + bouton ; **coût** → type **SST/Matériel/Intervention (+ Art. 1/2)**. Les noms d'artisans sont résolus via `useBatchResolver` (table `artisans`, `plain_nom`/`raison_sociale`). Pour le **coût en COST_UPDATE**, le `cost_type` n'est pas dans le payload (diff = `{amount}`) : il est résolu via la ligne `intervention_costs` à partir de **`related_entity_id`**, désormais exposé par `get_global_activity_feed` (**migration `99050`** — à appliquer via `supabase db push` pour activer le type sur les modifications de coût ; repli « Coût » générique tant que non poussée).
...
4. **Couleurs & avatars réels** — statuts via `useReferenceDataQuery` (couleurs CRM). Les **avatars** (couleur + **photo de profil**) sont résolus depuis la **source live `settings/team`** (`useGestionnaires`, soit `/api/settings/team`) — exactement comme la page Équipe — dans **la liste des gestionnaires ET le flux d'actions**, rendus via les primitives `Avatar` (photo + repli initiales colorées). Repli sur le snapshot dénormalisé du journal d'audit (`actor.*`) uniquement pour un acteur introuvable (ex. utilisateur archivé). Fini les avatars « figés » issus d'un instantané d'audit.
...
- `tests/unit/hooks/useDevMonitoringRealtime.test.ts` — abonnement audit-log + invalidation débouncée + nettoyage.
