...
>
> **Attribution à l'acteur (2026-09-09)** — une transition est comptée pour le gestionnaire qui l'a **effectuée** (`changed_by_user_id`), et non pour celui à qui le dossier est assigné (`assigned_user_id`). Un gestionnaire qui traite les dossiers d'un collègue absent en récolte les statistiques. Conséquence assumée : si deux acteurs passent le même dossier au même statut sur la période, la dédup n°2 s'applique par acteur et chacun compte 1. Les stats de **marge / CA** (`user/margin.ts`) et le **portefeuille** (`status-counts.ts`, `recent.ts`) restent, eux, attribués au propriétaire du dossier.
