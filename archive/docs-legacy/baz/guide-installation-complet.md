...
```bash
# Vérifier que tous les fichiers importants sont présents
ls -la scripts/
# Devrait contenir :
# - import-google-sheets-complete.js
# - create-auth-users.js
# - run-import.sh
# - clear-google-sheets-data.js
# - restore-auth-users.js

ls -la supabase/
# Devrait contenir :
# - config.toml
# - migrations/ (dossier avec les migrations SQL)

ls -la docs/baz/
# Devrait contenir :
# - guide-installation-complet.md (ce guide)
```
...
### 7.2 Exécuter l'import

#### 7.2.1 Vérifier la présence des scripts
```bash
...
**Option B : Script wrapper (recommandé)**
```bash
# Utiliser le script wrapper qui charge automatiquement les variables d'environnement
chmod +x scripts/run-import.sh

# Mode dry-run
./scripts/run-import.sh --dry-run

# Import réel
./scripts/run-import.sh

# Avec options
./scripts/run-import.sh --batch-size=50 --verbose
```
