-- ============================================================
-- Migration 99037: enrichissement de get_top_entities (vue Pulse → Dossiers actifs)
--
-- Ajoute, par dossier : la dernière action (type + auteur) et la répartition
-- par acteur (mix bar). Contrat existant conservé (count, last_action_at).
-- Réservé au rôle dev (guard interne). CREATE OR REPLACE — même signature.
-- ============================================================
...
BEGIN
  ...
  WITH base AS (
    SELECT 'intervention'::text AS entity_type, ial.intervention_id AS entity_id,
           i.reference_agence AS label, ial.occurred_at, ial.action_type,
