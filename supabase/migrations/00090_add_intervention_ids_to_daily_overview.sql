-- ========================================
-- Add intervention ID lists to get_team_daily_overview
-- ========================================
-- Version: 1.0
-- Date: 2026-02-17
-- Description: Enrichit get_team_daily_overview avec les listes
--   d'intervention_id + numero_associe pour chaque categorie
--   (creees, terminees, devis envoyes) afin de permettre un acces
--   rapide aux interventions depuis la page de suivi.
  ...
  RETURN (
    ...
    FROM (
      SELECT
        ...
        -- Liste des interventions creees (id + numero)
        (SELECT COALESCE(jsonb_agg(jsonb_build_object(
          'id', ial.intervention_id,
          'numero', i.numero_associe
        ...
        -- Liste des interventions terminees (id + numero)
        (SELECT COALESCE(jsonb_agg(jsonb_build_object(
        ...
        -- Liste des interventions devis envoyes (id + numero)
        (SELECT COALESCE(jsonb_agg(jsonb_build_object(
          'id', ist.intervention_id,
          'numero', i.numero_associe
        ) ORDER BY ist.transition_date DESC), '[]'::jsonb)
...
COMMENT ON FUNCTION public.get_team_daily_overview(date) IS
  'Retourne un apercu JSON de l''activite de toute l''equipe pour une journee donnee (incluant listes d''IDs intervention par categorie)';
