-- ========================================
-- Fix: status code TERMINEE/CLOTUREE -> INTER_TERMINEE
-- ========================================
-- Les RPCs filtraient sur to_status_code IN ('TERMINEE', 'CLOTUREE')
-- alors que le vrai code en base est 'INTER_TERMINEE'.
-- Corrige les 3 RPCs: get_user_daily_activity, get_team_daily_overview, get_team_weekly_stats.

-- ========================================
-- 1) get_user_daily_activity
-- ========================================
  ...
  RETURN (
    ...
    FROM (
      SELECT
        ...
        -- Liste des interventions creees
        (SELECT COALESCE(jsonb_agg(jsonb_build_object(
        ...
        -- Liste des interventions devis envoyes
        (SELECT COALESCE(jsonb_agg(jsonb_build_object(
          'id', ist.intervention_id,
          'numero', i.reference_agence
        ) ORDER BY ist.transition_date DESC), '[]'::jsonb)
